...
export default async function newProject() {
	...
	if (mAnswer.menu == 'react (with CRA)') {
		...
		if (answer.typescript == 'Yes') {
			...
			exec(
				`yarn create react-app ${pAnswer.projectName} --template typescript`,
				(error, stdout, stderr) => {
					if (error) {
			...
			exec(
				`yarn create react-app ${pAnswer.projectName}`,
				(error, stdout, stderr) => {
					if (error) {
						spinner.fail(`error: ${error.message}`);
						return;
		...
		if (answer.typescript == 'Yes') {
			...
			exec(
				`yarn create next-app --typescript ${pAnswer.projectName}`,
				(error, stdout, stderr) => {
					if (error) {
						spinner.fail(`error: ${error.message}`);
						return;
			...
			exec(
				`yarn create next-app ${pAnswer.projectName}`,
				(error, stdout, stderr) => {
					if (error) {
						spinner.fail(`error: ${error.message}`);
						return;
